...
NoInitVector<IndexType> radix_sort(
	...
	NoInitVector<IndexType> r_buf = idx;
	NoInitVector<IndexType> w_buf(idx.size());

	for (IndexType i = kmer - 1; ~i; i--) {
		auto bg = std::chrono::high_resolution_clock::now();
		radix_sort_key(S, K, r_buf, w_buf, i);
		auto ed = std::chrono::high_resolution_clock::now();
		cout << (ed - bg).count() << endl;
		w_buf.swap(r_buf);
	}
